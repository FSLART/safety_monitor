...
SafetyMonitor::SafetyMonitor() : Node("safety_monitor")
{
    ...
    mapping_sub = this->create_subscription<lart_msgs::msg::ConeArray>(cone_array_topic, 10, std::bind(&SafetyMonitor::check_freq_and_log, this, _1, 5, cone_array_topic));

    // create the subscriber for the planner
    planning_sub = this->create_subscription<nav_msgs::msg::Path>(path_topic, 10, std::bind(&SafetyMonitor::check_freq_and_log, this, _1, 5, path_topic));

    // create the subscriber for the control
    control_sub = this->create_subscription<ackermann_msgs::msg::AckermannDrive>(control_topic, 10, std::bind(&SafetyMonitor::check_freq_and_log, this, _1, 5, control_topic));
